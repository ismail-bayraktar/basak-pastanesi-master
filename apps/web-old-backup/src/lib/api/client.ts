    ...
    if (error.response) {
      ...
      switch (status) {
        ...
        case 404:
          ...
          break;
        case 429:
          // Rate limit exceeded - don't show toast, let the component handle retry
          // The error will be caught by the component's retry logic
          if (!isNotificationStream) {
            ...
            // Only show toast if it's not a slider request (sliderStore handles it)
            if (!error.config?.url?.includes('/api/slider/list')) {
              toast.error(message);
            }
          }
          break;
