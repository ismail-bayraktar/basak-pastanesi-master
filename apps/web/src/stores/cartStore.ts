  ...
  persist(
    (set, get) => ({
      ...
      getCartAmount: () => {
        ...
        for (const productId in items) {
          ...
          for (const size in items[productId]) {
            ...
            // Safety check for sizePrices
            const sizePrices = product.sizePrices || [];
            const sizePrice = sizePrices.find((sp) => sp.size === Number(size));
      ...
      getShippingFee: () => {
        ...
        // 1000 TL = 100000 kurus (assuming price is in kurus/cents)
        const minimumForFreeShipping = 1000 * 100;
    ...
    {
      name: 'tulumbak-cart-storage', // name of the item in the storage (must be unique)
